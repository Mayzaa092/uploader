...
export async function handleRequest(req, res) {
  ...
  try {
    ...
    if (pathname === '/api/shorten') {
      const respondWithShortLink = (original) => {
      ...
      if (req.method === 'POST') {
        ...
        return respondWithShortLink(original);
      }

      if (req.method === 'GET') {
        // GET /api/shorten?url=https://... — bikin short link langsung lewat query string
        const query = new URLSearchParams(fullUrl.split('?')[1] || '');
        const queryUrl = (query.get('url') || query.get('link') || '').trim();
        if (queryUrl) return respondWithShortLink(queryUrl);

        // GET /api/shorten tanpa query — list semua short link yang ada
    ...
    // ------------------------------------------------------------------
    // /api/clone — "clone" a file from a remote URL into this uploader.
    // Server downloads the URL itself and re-hosts the bytes, so the
    // caller never has to download+reupload manually.
    // ------------------------------------------------------------------
    if (pathname === '/api/clone' || pathname === '/api/clone/') {
      const MAX_CLONE_SIZE = 25 * 1024 * 1024; // 25 MB

      const respondWithClone = async (sourceUrl) => {
        if (!isValidUrl(sourceUrl)) {
          return send(res, 400, { author: AUTHOR, error: 'URL tidak valid' });
        }
        ...
        try {
          ...
          upstream = await fetch(sourceUrl, { signal: controller.signal, redirect: 'follow' });
          clearTimeout(timeout);
        } catch (err) {
          return send(res, 502, { author: AUTHOR, error: 'Gagal mengambil URL: ' + err.message });
        }

        if (!upstream.ok) {
          return send(res, 502, { author: AUTHOR, error: `URL sumber membalas HTTP ${upstream.status}` });
        }

        const declaredLength = Number(upstream.headers.get('content-length') || 0);
        if (declaredLength && declaredLength > MAX_CLONE_SIZE) {
          return send(res, 413, { author: AUTHOR, error: 'File terlalu besar (maks 25 MB)' });
        }
        ...
        if (!fileBuffer.length) {
          return send(res, 400, { author: AUTHOR, error: 'File dari URL tersebut kosong' });
        }
        if (fileBuffer.length > MAX_CLONE_SIZE) {
          return send(res, 413, { author: AUTHOR, error: 'File terlalu besar (maks 25 MB)' });
        }

        // Filename priority: Content-Disposition header > last URL segment > fallback id
        let filename = '';
        const disposition = upstream.headers.get('content-disposition') || '';
        const dispMatch = disposition.match(/filename\*?=(?:UTF-8'')?["']?([^"';\n]+)/i);
        if (dispMatch) {
          try { filename = decodeURIComponent(dispMatch[1]); } catch { filename = dispMatch[1]; }
        }
        if (!filename) {
          try {
        ...
        if (!filename) filename = `clone-${generateId()}`;
      ...
      if (req.method === 'POST') {
        ...
        const sourceUrl = (body.url || body.link || '').trim();
        if (!sourceUrl) return send(res, 400, { author: AUTHOR, error: 'Field "url" wajib diisi' });
        return respondWithClone(sourceUrl);
      }

      if (req.method === 'GET') {
        const query = new URLSearchParams(fullUrl.split('?')[1] || '');
        const sourceUrl = (query.get('url') || query.get('link') || '').trim();
        if (!sourceUrl) {
          return send(res, 400, { author: AUTHOR, error: 'Parameter "url" wajib diisi, contoh: /api/clone?url=https://...' });
        }
        return respondWithClone(sourceUrl);
      }
    }
